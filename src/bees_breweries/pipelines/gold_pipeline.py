...
class GoldBreweriesPipeline:
    def __init__(
        ...
        self.silver_path = Path(
            silver_path
            or Path(settings.SILVER_PATH) / "breweries" / "breweries.parquet"
        )
        self.storage = storage or GoldStorage()
    ...
    def run(self) -> Path:
        ...
        if not self.silver_path.exists():
            raise FileNotFoundError(
                f"Silver parquet not found: {self.silver_path}"
            )
        ...
        logger.info("Gold Breweries Pipeline finished successfully")
        return path
